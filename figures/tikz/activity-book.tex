...
\node[box] (StartApp) at (0, 7.2) {Access Appointment Section};

\node[box, text width=4cm] (Auth) at (3.8, 4.8) {Display Doctors List};

\node[box] (Enter) at (-3.8, 3.2) {Select Doctor \& Time};

\node[box] (Verify) at (3.8, 1.2) {Check Availability};

\node[diamond_node] (Dia) at (0, -0.8) {};

\node[box] (Error) at (-3.8, -0.8) {Display unavailable message};

\node[box] (Dash) at (3.8, -3.2) {Confirm Booking \& Save};
