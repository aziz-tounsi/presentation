...
\node[box] (StartApp) at (0, 7.2) {Trigger PRI Computation};

\node[box, text width=4cm] (Auth) at (3.8, 4.8) {Fetch Health Metrics};

\node[box] (Enter) at (-3.8, 3.2) {Extract Sleep \& Vitals};

\node[box] (Verify) at (3.8, 1.2) {Evaluate Data Completeness};

\node[diamond_node] (Dia) at (0, -0.8) {};

\node[box] (Error) at (-3.8, -0.8) {Prompt User to Sync Wearable};

\node[box] (Dash) at (3.8, -3.2) {Calculate Score \& Update View};
...
\draw[arrow] (Dia.west) -- node[above, font=\sffamily\small, yshift=-0.5mm] {incomplete} (Error.east);
\draw[arrow] (Error.north) -- (Enter.south);
\draw[arrow] (Dia) -- node[above right, font=\sffamily\small, xshift=-1mm, yshift=-1mm] {complete} (Dash);
\draw[arrow] (Dash) -- (End);
